...
| `agent-resource-plan` | Resource checks, Plan/Lock contracts, Plan Delta/staleness semantics, diffs, inventory, validation, and rendering. | Source-prepared as `0.1.0-preview.6`; verify the live `preview` tag after publishing. |
| `assetmason-cli` | Installed `assetmason` CLI for delegation preflight and local project-owned runtime workflows. | Source-prepared as `0.1.0-preview.6`; this package version includes the merged runtime command surface. |
| `agent-execution-profile` | Execution-profile contracts, host exports, locks, diffs, evidence reconciliation, handoff, and receipt validation. | Source-prepared as `0.1.0-preview.6`; verify the live `preview` tag after publishing. |

## Quickstart

After preview.6 is published:
...
The preview.6 CLI package contains the current local runtime mechanics, including project-owned run identity, isolated worktree support, checkpoints, pause/resume, handoff/receipt, scopes, governed decision memory, retry/fork lineage, and worker adapters.
...
Preview API note: semantic versioning may change during preview. `preview.6` is a prerelease rather than a stable release, and `latest` remains at its actual live registry value.
