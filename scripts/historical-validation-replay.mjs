...
const log = await execAsync("git log --no-merges --format='%H%x09%P%x09%s' -n " + String(Math.max(requestedCount + 10, 30)), { cwd: repoRoot });
...
try {
  ...
  for (const item of commits) {
    ...
    await execAsync(`git archive ${item.base_sha} -o "${archivePath}"`, { cwd: repoRoot });
    await execAsync(`tar -xf "${archivePath}" -C "${target}"`);
